...
#  These departures include a regional shock (6 total regions).  
# The user specifies the noise level.
...
nationalAvg = 0.50
regionShockSD = 0.02
regionshockSD = input("Independent across 6 regions, what is the regional shock? (e.g. 0.02) ")
nRegions = 6
print("I will assume that this accounts for 0.33 of di variability, so nonNoise x 0.67")
nnFactor = 0.67
...
# NEW: must read in each state's region
regionNames =["????"]*nRegions   #these are the names of the regions; to be read in 
regionID = ["??"]*nStates        #this connects the states to their regions
regionShock=[0.]*nRegions     #this will be the simulation-dependent shock in a region

for j in range(nStates) :   #read in which region is associated with this state
    currentRow = str(j+2)
    regionCell = ws["F"+currentRow]
    regionID[j] = regionCell.value
for n in range(nRegions) : #read in the region names
    currentRow = str(n+2)
    regionNameCell = ws["G"+currentRow]  # read in the list of region names (6 of them)
    regionNames[n] = regionNameCell.value
...
for k in range(nBetas) :
    for i in range(n_samples) :
        ...
        for n in range(nRegions) :  #NEW LOOP - assign some regional shocks
            regionShock[n] =np.random.normal(0.0,regionShockSD)

        for j in range(nStates) :  #loop to tentatively assign the delVi's
            regionIndex=regionNames.index(regionID[j]) #finding which #region we are in for this state
            nonnoise[j] = np.random.normal(0.0,stateSD[j])*nnFactor  #here, we sample the (di,sigma-i) space
            nonnoise[j] += regionShock[regionIndex] #add in the regional shock (SPECIAL FOR THIS CODE)
            # error[j] = np.random.normal(0.0,noise)
            sumOffset += stateEV[j]*( nonnoise[j] + avgDelta[j] )  #will be used in below normalization
        adjust = sumOffset/(maxEV)  #this sums all deviations from sum(di
...
for nVus in range(numberVus) : #each Vus interval ("bin") gets its own worksheet
    ...
    for i in range(noSims) :  #run a number of simulations near this Vus value
        ...
        for j in range(nStates) :  #loop to tentatively assign the delVi's
            ...
            nonnoise[j] = np.random.normal(0.0,stateSD[j])*nnFactor  #here, we sample the (di,sigma-i) space
            nonnoise[j] += regionShock[regionIndex] #add in the regional shock (SPECIAL FOR THIS CODE)
            error[j] = np.random.normal(0.0,noise)  #no nnFactor here as errors are not regionally correlated
